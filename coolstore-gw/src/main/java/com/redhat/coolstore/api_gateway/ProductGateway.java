    ...
    public void configure() throws Exception {
        ...
        .get("/").description("Get product catalog").outType(Product.class)
            .route().id("productRoute")
                ...
                .recipientList(simple("http4://{{env:CATALOG_ENDPOINT:catalog:8081}}/api/products")).end()
                .onFallback()
        ...
        from("direct:inventory")
            ...
            .hystrix().id("Inventory Service")
                ...
                .recipientList(simple("http4://{{env:INVENTORY_ENDPOINT:inventory:8081}}/api/availability/${header.itemId}")).end()
            .onFallback()
                //.setHeader(Exchange.HTTP_RESPONSE_CODE, constant(Response.Status.SERVICE_UNAVAILABLE.getStatusCode()))
        ...
        from("direct:rating")
                ...
                .setHeader(Exchange.HTTP_URI, simple("http://{{env:RATING_ENDPOINT:rating:8081}}/api/rating/${header.itemId}"))
